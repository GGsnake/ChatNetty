...
//		user.setSalt(Encodes.encodeHex(salt));
        byte[] hashPassword = Digests.sha1(user.getPassword().getBytes(), null, Constants.HASH_INTERATIONS);
     ...
     * 设定安全的密码，生成随机的salt并经过1024次 sha-1 hash
     */
    public static String enpPassword(String password) {
//		byte[] salt = Digests.generateSalt(Constants.SALT_SIZE);
//		user.setSalt(Encodes.encodeHex(salt));
        if (StringUtils.isBlank(password)) {
            return null;
        }
        byte[] hashPassword = Digests.sha1(password.getBytes(), null, Constants.HASH_INTERATIONS);
...
//	}
    ...
    public static void main(String args[]) throws Exception {
        ...
        //}
//		String admin = entryptPassword("admin", "2b97125d54070d51");
